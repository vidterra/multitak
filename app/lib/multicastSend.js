...
messageEmitter.on('cotMulticastSend', (message) => {
	if(message.source.type === 'multicast' && helper.getInterfaceAddresses().includes(message.source.ip)) return // if the source was multitak IP don't send it back to multicast
